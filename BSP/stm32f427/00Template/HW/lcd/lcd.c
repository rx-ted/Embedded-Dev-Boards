...
// 搜索lcd型号所有的可能
device_result_code_t check_lcd_setup_param_by_id()
{
...
void lcd_init(void)
{
    ...
    if (check_lcd_setup_param_by_id() != DEVICE_SUCCESS)
    {
        return;
    }
...
/* 设置屏幕显示方向 */
device_result_code_t set_lcd_display_direction(lcd_display_dir_t dir, lcd_display_mode_t mode)
{
    device_result_code_t res = DEVICE_SUCCESS;
